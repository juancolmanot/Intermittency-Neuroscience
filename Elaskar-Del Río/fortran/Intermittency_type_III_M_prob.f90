program Intermittency_type_III_M_prob
    use precision
    use mtmod
    use maps
    use matrixalgebra
    ...
    integer(sp)                 :: seed
    integer(dp)                 :: i,j,k
    integer(dp)                 :: n_bins,n_iterations
    ...
    ! Open files to write
    open(21,file='Intermittency_type_III_x_trajectory1.dat',status='replace')
    open(22,file='Intermittency_type_III_x_trajectory2.dat',status='replace')
    open(23,file='Intermittency_type_III_M_prob1.dat',status='replace')
    open(24,file='Intermittency_type_III_M_prob2.dat',status='replace')
    ...
    n_iterations = 1000000_dp ! Number of iterations
    ...
    do i = 1,2
        ...
        do j = 1,n_bins
            if (counter(j) > 0_dp) then
                ...
                prob(j) = real(counter(j)/real(n_iterations,dp),dp)
...
end program Intermittency_type_III_M_prob