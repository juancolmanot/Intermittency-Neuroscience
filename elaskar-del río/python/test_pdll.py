...
l_count_2 = l_count[l_count != 0]
l_bins_2 = np.linspace(min(l_count_2), max(l_count_2), len(l_count_2))

l_expected = 0

for i in range(n_bins_l):

    l_expected += l_avg[i] * l_count[i]


print(np.log(l_expected))

fig, ax = plt.subplots(1, 2)

ax[0].plot(l_bins_2, l_count_2, color='black')
ax[0].set_xlabel('x')
ax[0].set_ylabel('p(l)')

ax[1].plot(x_bins, x_count, color='black')
ax[1].set_xlabel('x')
ax[1].set_ylabel('RPD(x)')
