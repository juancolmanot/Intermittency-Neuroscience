...
!	Archives and formats
	...
	open (unit=62,file='funcion M de reinyecci�n _ alternativa 1.dat',status='unknown', form='formatted', access='sequential')
	...
	open (unit=137,&
		file='longitud laminar media estadistica (0,i) programa.dat', &
		status='unknown', form='formatted', access='sequential')

	open (unit=138, & 
		file='longitud laminar media estadistica (0,i) ezequiel.dat', &
		status='unknown', form='formatted', access='sequential')
    ...
    open (unit=162,file='funcion M de reinyecci�n _ alternativa 2.dat',status='unknown', form='formatted', access='sequential')
...
!	print *, 'xr =', xr
	...
	if (inec .eq. 1) then
	  ...
	  do i = 1, 1501
	    ...
	    if(y .gt. 1.d0) then
		  y = (y-1.d0)**gamma
		  yder = gamma * (((1.d0+epsi)*x + (1.d0-epsi)*(x**lambda -1.d0))**(gamma-1.)) &
		  * ((1.d0+epsi) + lambda*(1.d0-epsi)*(x**(lambda-1.)))    
        end if 
...
!
	...
	do i = 1,ns
	  if(probgen(i) > 0.0) then
        ...
        probgen(i) = probgen(i)/(1.d0*ntotiter) !probabilidad total general en cada subintervalo sin considerar si hay reinyecci�n
	  end if
    ...
    do j = 1,ns
	   ...
	   write (178,400) j, xi(j), fmpgen(j) 	!funci�n M total (alternativa 1)

	   write (180,400) j, xi(j), fmpagen(j)	!funci�n M total (alternativa 2)
...
!	  valm(i) = valm(i-1) + val(i)*prob(i)*delta ! valor medio en el intervalo (0,i)
	...
	do i = 1,ns
      write (50,200) xi(i), prob(i)	 !funcion probabilidad de reinyecci�n en el intervalo i
      write (51,200) xi(i), probr(i) !funcion probabilidad real de reinyecci�n en el intervalo i
...
!	  if(xi(j) .ge. 0.0) then
	     ...
	     if (mfun .gt. 0.0) then
		   fmp(j) = vaz/mfun   !funcion M de reinyecci�n, integra entre el punto de equilibrio y el limite superior
		                       ! ALTERNATIVA 1 
	     end if
...
!   CALCULO ALTERNATIVO DE Mprima
    ...
    do j = mns-1,ns
	     ...
	     if (mfuna .gt. 0.0) then
		   fmpa(j) = vaza/mfuna   !funcion M de reinyecci�n, integra entre el punto de equilibrio y el limite superior
		                          ! ALTERNATIVA 2
	     end if
