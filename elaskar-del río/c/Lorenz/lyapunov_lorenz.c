...
int main(void) {
    ...
    FILE *bifurcation_lorenz = fopen("../datafiles/bifurcation_lorenz_145_149.dat", "w");
    ...
    b_param = gsl_vector_alloc(Nb);

    b_param = gsl_linspace(145, 149, Nb);

    for (unsigned int k = 0; k < Nb; k++) {

        p->a = 10.0;
        p->b = gsl_vector_get(b_param, k);
        p->c = 8.0 / 3.0;
            ...
            if (t > 1000.0) {
                if (xi[1] > xp) {
                    if (xi[0] < xp) {
                        ...
                        if (counter % 10 == 0) {
                            fprintf(bifurcation_lorenz, 
                            "%5.8f %5.8f\n", gsl_vector_get(b_param, k), yreg[counter]);
                        }
                        yprev = yreg[counter];
                        zprev = zreg[counter];
                        counter++;
